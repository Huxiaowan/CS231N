...
def batchnorm_backward(dout, cache):
    ...
    dsample_var = np.sum(-1.0 / 2 * dx_normalized * (x -
                                                     sample_mean) / (sample_var + eps)**(3.0 / 2), axis=0)
    dsample_mean = np.sum(-1 / np.sqrt(sample_var + eps) * dx_normalized, axis=0) + \
        1.0 / N * dsample_var * np.sum(-2 * (x - sample_mean), axis=0)
    dx = 1 / np.sqrt(sample_var + eps) * dx_normalized + dsample_var * \
        2.0 / N * (x - sample_mean) + 1.0 / N * dsample_mean
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    for n in range(N):
        for c in range(C):
            x_pad[n, c] = np.pad(x[n, c], (1, 1), 'constant',
                                 constant_values=(0, 0))
    ...
    for n in range(N):
        for i in range(H_out):
            for j in range(W_out):
                for f in range(F):
                    current_x_matrix = x_pad[n, :, i * stride: i *
...
def conv_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            x_pad[n, c] = np.pad(
                x[n, c], (1, 1), 'constant', constant_values=(0, 0))
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dx_pad[n, :, i * stride: i * stride + HH, j *
                           stride:j * stride + WW] += w[f] * dout[n, f, i, j]
